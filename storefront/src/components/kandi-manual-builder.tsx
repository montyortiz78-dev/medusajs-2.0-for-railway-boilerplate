...
function PaletteItem({ color, name }: { color: string, name: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
        ...
        {...attributes}
        className={clx(
            "group relative w-10 h-10 rounded-full border border-ui-border-base shadow-sm transition-all ring-2 ring-transparent hover:ring-ui-fg-interactive touch-none",
...
export default function KandiManualBuilder({ pattern, setPattern }: Props) {
  ...
  // Setup the work area as a droppable zone so we can drop new beads into empty space
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // If dropped nowhere valid, do nothing
    if (!over) return;
    ...
    if (active.data.current?.type !== 'palette') {
        if (active.id !== over.id) {
            ...
            // If dragging a bead onto the work-area background, don't move it (optional: move to end?)
            // For now, we only reorder if over another bead.
            if (newIndex !== -1) {
    ...
    // Case 2: Dropping Palette Item
        ...
        // If dropped over the work area container OR any existing bead
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 w-full">
        ...
        {/* 1. The String (Sortable + Droppable Area) */}
        <div 
            ref={setWorkAreaRef} // Make the container a drop target
            className={clx(
                "bg-ui-bg-subtle border border-ui-border-base rounded-xl p-4 min-h-[100px] relative transition-colors",
                // Visual cue when dragging over empty area
